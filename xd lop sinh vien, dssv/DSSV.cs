...
namespace QLSV
{
    class DSSV
    {
        ...
        public void doc()
        {
            XmlTextReader read = new XmlTextReader("danhsach.xml");
            while (read.Read())
            {
                if (read.NodeType == XmlNodeType.Element && read.Name == "SV")
                {
                    sinhvien sv = new sinhvien();
                    while (read.Read())
                    {
                        ...
                        {
                            string elementName = read.Name;
                            read.Read();
                            switch (elementName)
                            {
                                case "ID":
                                    sv.Id = int.Parse(read.Value);
                                    break;
                                case "NAME":
                                    sv.Name = read.Value;
                                    break;
                                case "TOAN":
                                    sv.Toan = double.Parse(read.Value);
                                    break;
                                case "LY":
                                    sv.Ly = double.Parse(read.Value);
                                    break;
                                case "HOA":
                                    sv.Hoa = double.Parse(read.Value);
                                    break;
                            }
                        }
                        if (read.NodeType == XmlNodeType.EndElement && read.Name == "SV")
                        {
                            sv.Tb = sv.trungbinh();
                            sv.Xl = sv.Xeploai();
                            listSV.Add(sv);
